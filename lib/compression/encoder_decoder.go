...
import (
	...
	"iden69/archiver/lib/compression/utils"
	"log"
	"strings"
	"unicode"
)
...
func New(tblGen table.Generator) EncoderDecoder {
...
func parseFile(data []byte) (table.EncodingTable, string) {
	...
	body := utils.NewBinChunks(data).Join()
...
func buildEncodedFile(tbl table.EncodingTable, data string) []byte {
	...
	buf.Write(utils.SplitByChunks(data, utils.ChunkSize).Bytes())
...
// encodes str into binary codes string without spaces
...
// is opposite to prepareText, it prepares decoded text to export:
